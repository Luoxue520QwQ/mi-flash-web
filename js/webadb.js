...
(function(root, factory) {
	...
	Adb.WebUSB.Device.prototype.rebootToFastboot = function() {
		return Adb.Stream.open(this, "reboot:bootloader");
	};
	...
	Fastboot.WebUSB.Device.prototype.flash = function(partition, data) {
		let device = this;
		let chunkSize = device.max_datasize;
		let offset = 0;
		let totalSize = data.byteLength;

		return device.send("download:" + totalSize.toString(16))
			.then(() => {
				return device.sendPartitionData(data, chunkSize, offset, totalSize);
			})
			.then(() => {
			...
			.then(response => {
				...
				let status = decoder.decode(response);
				if (!status.startsWith("OKAY")) {
					throw new Error("Download failed: " + status);
				}
			})
			.then(() => {
				return device.send("flash:" + partition);
			})
			.then(() => {
				return device.receive();
			})
			.then(response => {
				...
				let status = decoder.decode(response);
				if (!status.startsWith("OKAY")) {
					throw new Error("Flash failed: " + status);
				}
			});
	};

	// Helper function to send partition data in chunks
	Fastboot.WebUSB.Device.prototype.sendPartitionData = function(data, chunkSize, offset, totalSize) {
		let device = this;
		let end = Math.min(offset + chunkSize, totalSize);
		let chunk = data.slice(offset, end);

		return device.send(chunk)
			.then(() => {
				return device.receive();
			})
			.then(response => {
				...
				if (!status.startsWith("DATA")) {
					throw new Error("Data transfer failed: " + status);
				}
				offset = end;
				if (offset < totalSize) {
					return device.sendPartitionData(data, chunkSize, offset, totalSize);
